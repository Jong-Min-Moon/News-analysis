...
for new_stopword in ['네이버', '뉴스', '구독', '뉴스1', '파이낸셜뉴스', '연합뉴스', '원보', '원본', '무단', '보기', '배포', '저작권자', '금지', '이날', '전날', '가능', '이후', '로이터', '아사히', '데일리NK', '주간지', '기사', '시사']:
...
def tokenize(sent):
    ...
    words = [word + ('다' if tag.startswith('V') else '') # 동사에는 '다'를 붙여줌
            ...
            if tag.startswith('N') and word not in stopwords and len(word) > 1] # 조사, 어미, 특수기호는 제거

    # if '기자' in words:
    #     first_occur_giza = words.index('기자')
    #     last_occur_giza  = list_rindex(words, '기자')
    #     print(first_occur_giza, last_occur_giza)
    #     return words[first_occur_giza + 1 : last_occur_giza - 1]
    # else:
    return words



def scan_vocabulary(sents, tokenize, min_count=2, max_count = 0.8):
    #모든 단어를 쭉 나열
...
def cooccurrence(tokens, vocab_to_idx, window=2, min_cooccurrence=2):
    ...
    mat = dict_to_mat(counter, n_vocabs, n_vocabs)
    print(mat)
    return mat
...
def textrank_keyword(sents, tokenize, min_count, window, min_cooccurrence, df=0.85, max_iter=30, topk=30):
	...
	R = pagerank(g, df, max_iter).reshape(-1)#벡터화
   
	#argsort로 오름차순으로 idx를 정렬. 값 큰 것을 원하니 -topk부터 마지막 원소까지.
	idxs = R.argsort()[-topk:] 
		#내림차순으로 정렬하기 위해 reversed 사용
