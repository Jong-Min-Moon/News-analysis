...
def do_crawl(keyword, dt, initialize):
    ...
    mycrawl.df_naver_news.to_sql('naver_news_1', con, if_exists = 'append', index = False)
    con.commit()

    mycrawl.get_naver_news_comment()
    NLP.add_sent_score(mycrawl.df_naver_news_comment).to_sql('naver_comment_1', con, if_exists = 'append', index = False)

    con.commit()


    #doc id를 join해서 붙임
    query = 'UPDATE naver_comment_1 SET doc_id = (SELECT doc_id FROM naver_news_1 WHERE in_url = naver_comment_1.url) WHERE doc_id IS NULL'  
